...
public class InformeControllerWebTestClientIT extends AbstractIntegration {
    ...
    @Test
    @DisplayName("Crear informe de una imagen y verificar la predicción")
    void crearInforme_y_VerificarPrediccion() {
        Informe nuevoInforme = new Informe();
        nuevoInforme.setContenido("Revisión de rutina");
        ...
        testClient.get().uri("/informe/imagen/" + imagen.getId())
                ...
                .consumeWith(informeResponse -> {
                    Informe inf = informeResponse.getResponseBody().get(0);

                    String prediccion = inf.getPrediccion();
